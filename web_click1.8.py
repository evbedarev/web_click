...
class find_and_click():
	...
	def __init__(self,text_find,num_link,driver):
		...
		self.find_page(driver)

		

	def find_page(self,driver):
		d = []
		d = driver.find_elements_by_class_name("pager__item_kind_page")
		self.five_links(driver,1,1)
		time.sleep(2+random.random()+random.random()+random.random())
		for i in range(1,d.__len__()):
			if self.link_by_name(' / 9colours.ru',driver):
				break
			time.sleep(2+random.random()+random.random()+random.random())
			d[i].click()
			time.sleep(2+random.random()+random.random()+random.random())
	...
	def link_by_name(self,site_name,driver):
		my_link = driver.find_elements_by_xpath("//*[contains(text(), '%s')]" % site_name)
		time.sleep(2)
		if my_link.__len__()>0:
			my_link[0].click()
			time.sleep(2)
	...
	def find_links(self,driver):
		...
		for i in range(1,links.__len__()):
			if links[i].is_displayed():
				...
				if vis_links.__len__() > 50:
					break
			else:
				print("element not found")
		print(vis_links.__len__())
...
chrome_options.add_argument(r"user-data-dir=C:\Documents and Settings\Admin\Local Settings\Application Data\Google\Chrome\User Data")
